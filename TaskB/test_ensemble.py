...
if __name__ == "__main__":
    ...
    path = "wandb/ensembler/*/files/scibert_crf_f1.pt"
    index = 0
    directory = glob.glob(path)
    for i, path in enumerate(directory):
    	print(i, path)
    for model_path in directory:
	    model.load_state_dict(torch.load(model_path))
	    model.to(device)
	    tokenized, predicted = eval_fn(test_data_loader, model, device)
	    df[f'predicted_tag_{index}'] = process(sentences, tokenized, predicted)
	    index += 1
    
    
    #df['predicted_tag'] = predicted_tag
    df.to_csv("test_results_154.csv", index = None)
