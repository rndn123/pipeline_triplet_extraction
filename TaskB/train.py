...
df = pd.read_csv("../Preprocessed_Dataset/NCG_phrases.csv")
df.columns = ['Sentence #', "Word", "Tag", "POS"]

df.loc[:, "POS"] = enc_pos.fit_transform(df["POS"])
df.loc[:, "Tag"] = enc_tag.fit_transform(df["Tag"])

df_scierc = pd.read_csv("../Preprocessed_Dataset/scierc_sent_phrases.csv")
...
df_sciclaim = pd.read_csv("../Preprocessed_Dataset/SciClaim.csv")
...
df_tdm = pd.read_csv("../Preprocessed_Dataset/TDMSci_dataset.csv")
...
def training():
	...
	model.to(device)
	torch.save(model.state_dict(), "scibert_crf_f1.pt")
	#torch.save(model.state_dict(), "initializer.pt")
	...
	for epoch in range(config.EPOCHS):
		...
		scierc_loss = engine.train_fn(scierc_data_loader, model, optimizer, device, scheduler, wandb_config)
		#tdmsci_loss = engine.train_fn(tdmsci_data_loader, model, optimizer, device, scheduler, wandb_config)
		sciclaim_loss = engine.train_fn(sciclaim_data_loader, model, optimizer, device, scheduler, wandb_config)
...
#sweep_id = wandb.sweep(sweep_config, project="SCIERC")
